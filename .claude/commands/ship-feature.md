...
**Plan before executing (mandatory):** the `feature` workflow starts with the Planner. Nothing in the project changes until the user explicitly approves the Planner's spec. If a scope change comes up later, the run goes back to planning for re-approval. Nothing is ever committed or pushed.
