...
- **Git workflow.** **Decided by the maintainer (2026-09-16):** work happens on a feature branch and lands on `master` by **squash merge**, one commit per backlog item or feature. The squashed commit's subject names the backlog item ID where there is one, e.g. `… (RISK-2)`, so `git log --grep` resolves an item to the change that closed it. Because squashing discards the branch's intermediate commits, write the squashed message to stand on its own: what changed and why, not "address review". Squashing also rewrites hashes, which is why the backlog records item IDs rather than commit hashes (`specs/_system/backlog.md:3`). Delete the branch once it has landed. **Branch before the work starts, not after** (2026-09-17): a dev-pipeline run creates `pipeline/<slug>` at step 3.4 before any persona runs and commits a checkpoint after each stage that changes files, so a run's output is never left uncommitted on `master` and a bad stage can be reset to the previous checkpoint rather than to the run's start. Only the orchestrator touches git, and only additively — branch, checkpoint, and a squash merge the maintainer asks for; personas may read git but never change it (`.claude/commands/run-workflow.md`, step 0). Backlog items are no longer marked `in progress` before a run: the branch is that signal, and the item goes `open` → `done` in the squash commit.
