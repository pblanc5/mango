...
| `src/content/frontmatter.rs` (118) | Splits the `---` JSON `---` block from the body; ignores a leading BOM; rejects unknown keys, listing every unknown and missing key in one error (RISK-4) | yes (`src/content/frontmatter.rs` 6 unit tests; `tests/build.rs` `build_accepts_utf8_bom_before_frontmatter`) |
...
| `src/content/frontmatter.rs` `MangoFrontmatter` (`:8-16`) | **CRLF untested.** Unknown keys are rejected since RISK-4 (`check_keys`). CRLF line endings are normalized by `lines()` and `join("\n")` (`:25`, `:35-36`), but no test uses `\r\n`. |
