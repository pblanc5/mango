...
| `src/content/frontmatter.rs` | Splits the `---` JSON `---` block from the body; ignores a leading BOM; rejects unknown keys, listing every unknown and missing key in one error (RISK-4) | yes (`src/content/frontmatter.rs` unit tests; `tests/build.rs` `build_accepts_utf8_bom_before_frontmatter`) |
