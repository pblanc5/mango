...
Resolved by `specs/risk-10/`: files and folders whose names start with `.` are skipped under both `site/` and `meta/assets/`, at any depth, together with everything inside a hidden folder, and the `--site` and `--assets` folders themselves are never skipped. A skipped entry is not resolved, read or checked, so a hidden broken symlink such as an Emacs lock file no longer fails the build; this reverses RISK-5's no-exemption choice for hidden names. Names starting with `_` are unaffected. The rule is one function, `hidden::is_hidden`, applied first in `loader::traverse` and `assets::collect`. `load_publishes_hidden_markdown_file` was replaced by `load_skips_hidden_files_and_folders`. Breaking, recorded in `CHANGELOG.md`. `tests/build.rs::build_skips_hidden_entries` proves that a site with hidden entries builds byte-identically to the same site without them.
