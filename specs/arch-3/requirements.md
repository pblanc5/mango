...
- AC-8.3 THE `README.md` file-name rule (`README.md:68`) and the `CLAUDE.md` "File names are strict" paragraph SHALL state that a segment made only of dots is rejected.
  **No longer applies to `README.md` since 2026-09-26 (`specs/risk-10/requirements.md`, AC-8.1):** a dot-only name starts with `.`, so the content loader skips it as hidden and the README no longer says it is rejected; it describes hidden entries instead. The `CLAUDE.md` "File names are strict" paragraph still states the rule, as the slug type's own invariant, and says the loader skips such names.
...
| 2026-09-26 | 20260926-163650-risk-10-skip-hidden-entries | Noted under AC-8.3 that its `README.md` half no longer applies: the README's dot-only sentence was removed by `specs/risk-10/requirements.md` AC-8.1. The `CLAUDE.md` half still holds. The original wording is kept. |
